<script lang="ts">
  ...
  export let schema: ZodString = z.string().regex(
    new RegExp('((?=.*\\d)|(?=.*\\W+))(?![.\\n])(?=.*[A-Z])(?=.*[a-z]).*$'),
...
<div class="d-flex align-items-center mb-3">
  <div class="flex-grow-1 bg-secondary bg-active-success rounded h-5px me-2 {indToDisplay > 0 ? 'active': ''}"></div>
  <div class="flex-grow-1 bg-secondary bg-active-success rounded h-5px me-2 {indToDisplay > 1 ? 'active': ''}"></div>
  <div class="flex-grow-1 bg-secondary bg-active-success rounded h-5px me-2 {indToDisplay > 2 ? 'active': ''}"></div>
  <div class="flex-grow-1 bg-secondary bg-active-success rounded h-5px {indToDisplay > 3 ? 'active': ''}"></div>
</div>

{#if error}
  <InputError error="{error}" errorMessages="{errorMessages}" />
{:else}
  <div class="text-muted">Use 8 or more characters with a mix of small & big letters, numbers or symbols.</div>
{/if}
